...
def start(message):
    ...
    btn2 = types.KeyboardButton("Получить подсказку")
    markup.add(btn1, btn2)
    bot.send_message(message.chat.id, text="Привет, {0.first_name}! Я алкобот! ".format(message.from_user), reply_markup=markup)
...
def first(message):
    ...
    elif (message.text == "Получить подсказку"):
